...
'
Identifies CAR-specific markers, comparing average expression of CAR.1 vs. CAR.2
If CAR.2 is NULL, compares to all other CARs
'
CAR_markers <- function(seurat_obj, CAR.1, CAR.2 = NULL, ...) {
  markers <- ident_markers(seurat_obj = seurat_obj, group.by = 'CARID', ident.1 = CAR.1, ident.2 = CAR.2, ...)
} 
...
compute_pb_avg <- function(seurat_obj, assay = 'RNA', group.by = 'CARID', 
                           features = NULL, slot = 'data',
                           transpose = F, seurat_scale = T, scale = T) {
...
Extra parameters passed into ComplexHeatmap::Heatmap()
'
DoAverageHeatmap <- function(seurat_obj, assays = 'RNA', group.by = 'CARID', features = NULL, slot = 'data',
                             return.Z.scores = F, transpose = F, gene_features = T, 
                             scale = T, ...) {
